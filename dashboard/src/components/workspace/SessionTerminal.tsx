...
// The xterm "fleet console" theme — shared with the legacy TerminalView.
const theme = {
  background: '#09090b',
  foreground: '#e4e4e7',
  cursor: '#34d399',
...
// Attaches xterm to the long-lived /ws/session socket. Unlike the legacy
// TerminalView, closing this does NOT kill the process — the hub keeps it alive
// and replays scrollback on reattach. We manage the socket directly here (rather
// than useSessionSocket) so xterm sizing and the first resize stay in lockstep.
export function SessionTerminal({ sessionId }: Props) {
  ...
  useEffect(() => {
    ...
    ws.onmessage = (ev) => {
      ...
      if ((msg.type === 'replay' && msg.kind === 'terminal' && msg.data) || (msg.type === 'output' && msg.data)) {
