...
import { rankByFit, fitScore } from './data'
...
export function NewSession({
  ...
  fleet: Machine[]
  onClose: () => void
  onStart: (machineId: string, name: string, type: string) => void
}) {
  const ranked = useMemo(() => rankByFit(fleet), [fleet])
  const firstPlaceable = ranked.find((m) => !m.offline) || ranked[0]
  const [type, setType] = useState('claude')
  const [name, setName] = useState('')
  const [pick, setPick] = useState(firstPlaceable.id)
              ...
              >
                  ...
                  {m.offline ? (
                    <div className="stats">
                      <div className="stat">
                        <span className="v" style={{ color: 'var(--fg-3)' }}>
                          Offline · wake in Fleet to place here
                        </span>
                      </div>
                    </div>
                  ) : (
                    <div className="stats">
                      <div className="stat">
                        ...
                        <span className="v good">{freeRAM.toFixed(0)} GB</span>
                      </div>
                      <div className="stat">
                ...
                <div className="fit">
                  ...
                  <div className="fitbar">
                    <i style={{ width: (m.offline ? 0 : score) + '%' }} />
                  </div>
                </div>
              </div>
            )
