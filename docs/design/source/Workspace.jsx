...
function CodeMesh({ activeFile }) {
  const L = (n, sel, ...kids) => (
    <div key={n} className={sel ? "selln" : ""}><span className="ln">{n}</span>{kids}</div>
  );
  return (
    <div className="code">
  ...
  return (
    <div className="term">
      <div className="term-h">
        ...
        <span className="mono" style={{ fontSize: 11, color: "var(--fg-3)" }}>studio-mbp · 2h 14m</span>
        <div style={{ marginLeft: "auto", display: "flex", gap: 4 }}>
          <button className="iconbtn" style={{ width: 26, height: 26 }}><Icon name="copy" size={14} /></button>
          <button className="iconbtn" style={{ width: 26, height: 26 }}><Icon name="maximize-2" size={14} /></button>
        </div>
      </div>
...
function Editor({ activeFile, onSelectFile, onStart }) {
      ...
      <div className="etabs">
        {tabs.map(t => (
          <div key={t} className={`etab ${t === activeFile ? "on" : ""}`} onClick={() => onSelectFile(t)}>
            ...
            {t}
            <span className="x"><Icon name="x" size={12} /></span>
          </div>
        ))}
        <div className="etab etab-spacer">
          <button className="iconbtn" style={{ width: 28, height: 28 }} title="New session" onClick={onStart}><Icon name="plus" size={14} /></button>
          <button className="iconbtn" style={{ width: 28, height: 28 }}><Icon name="git-branch" size={14} /></button>
        </div>
      </div>
      <CodeMesh activeFile={activeFile} />
      <Terminal />
      <div className="editor-status">
        <span className="seg-g"><Dot status="live" />studio-mbp</span>
        <span className="seg-g"><Icon name="git-branch" size={12} color="var(--fg-3)" />main</span>
        <span className="sp" />
...
function ToolCall({ icon, label, detail, status }) {
  return (
    <div className="tool">
      <div className="tool-h">
        ...
        <span className={`st ${status === "ok" ? "ok" : "run"}`}>
          {status === "ok"
            ? <React.Fragment><Icon name="check" size={12} />done</React.Fragment>
            : <React.Fragment><Icon name="refresh-cw" size={12} style={{ animation: "spin 1s linear infinite" }} />running</React.Fragment>}
        </span>
      </div>
...
function AIChat({ width }) {
  const [messages, setMessages] = React.useState([
    { who: "user", text: "Add a fallback in place() — if no machine is idle, hand the job to any reachable node, and only throw if the mesh is fully dark." },
    {
      who: "ai",
      tools: [
  ...
  const send = () => {
    ...
    setTimeout(() => {
      ...
      setMessages(m => [...m, {
        who: "ai",
        tools: [{ icon: "zap", label: "Placed session on <b>studio-mbp</b>", status: "ok", detail: "warmest reachable node · survives sleep + disconnect" }],
  ...
  return (
    <div className="chat" style={{ width, flex: "none" }}>
      <div className="chat-h">
        <span className="av"><Icon name="sparkles" size={14} /></span>
        <div style={{ display: "flex", flexDirection: "column" }}>
          <span className="nm">Claude</span>
          <span className="on-node">paired on studio-mbp · mesh.ts</span>
        </div>
        <button className="iconbtn" style={{ marginLeft: "auto" }}><Icon name="ellipsis" size={18} /></button>
      </div>
      ...
      <div className="chat-body" ref={bodyRef}>
        ...
        {thinking && <div className="thinking"><span className="d" />thinking on studio-mbp…</div>}
      </div>

      <div className="chat-foot">
        <div className="composer">
          <textarea
            rows={2} placeholder="Ask Claude, or describe a change…"
            value={val} onChange={e => setVal(e.target.value)}
            onKeyDown={e => { if (e.key === "Enter" && !e.shiftKey) { e.preventDefault(); send(); } }}
          />
          <div className="composer-row">
            <div className="left">
              <button className="iconbtn" style={{ width: 28, height: 28 }}><Icon name="folder" size={15} /></button>
              <button className="iconbtn" style={{ width: 28, height: 28 }}><Icon name="terminal" size={15} /></button>
              <Chip kind="ghost">mesh.ts</Chip>
            </div>
            <button className="send" onClick={send} disabled={!val.trim()}><Icon name="send" /></button>
          </div>
        </div>
      </div>
    </div>
...
function Workspace({ activeFile, onSelectFile, onStart }) {
  const wrapRef = React.useRef(null);
  const [chatW, setChatW] = React.useState(384);
