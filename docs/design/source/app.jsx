...
function Toast({ text, kind }) {
  return (
    <div className={`toast ${kind === "wake" ? "wake" : ""}`}>
      <Dot status={kind === "wake" ? "starting" : "live"} />{text}
    </div>
  );
}
...
function App() {
  ...
  const [selected, setSelected] = React.useState("garage-pc");
  const [activeFile, setActiveFile] = React.useState("mesh.ts");
  const [fleet, setFleet] = React.useState(() => FLEET.map(m => ({ ...m, sessions: [...m.sessions] })));
  ...
  const wake = (id) => {
    const m = fleet.find(x => x.id === id); if (!m) return;
    setView("fleet"); setSelected(id);
    ...
    clearTimeout(toastTimer.current);
    setTimeout(() => {
      setFleet(f => f.map(x => x.id === id
        ? { ...x, status: "idle", cpu: 3, memUsed: 4, net: "0.3 GB/s", uptime: "0m" }
        : x));
      flash(`${m.label} is awake → woven into the mesh`, "live");
    }, 2600);
  };

  const startSession = (machineId, name, type) => {
    const m = fleet.find(x => x.id === machineId);
    setFleet(f => f.map(x => x.id === machineId
      ? { ...x, status: "live", cpu: Math.max(x.cpu, 24), sessions: [...x.sessions, { name, status: "live", dur: "0m" }] }
      : x));
    setModal(false); setSelected(machineId); setView("workspace");
  ...
  return (
    <div className="app">
      <Rail
        view={view} fleet={fleet} selected={selected} activeFile={activeFile}
        onSelect={(id) => { setSelected(id); setView("fleet"); }}
      ...
      <div className="stage">
        {view === "fleet"
          ? <Fleet
              fleet={fleet} selected={selected} onSelect={setSelected}
              onStart={() => setModal(true)} onWake={wake}
