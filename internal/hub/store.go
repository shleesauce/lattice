...
func (s *Store) UpsertSession(rec SessionRecord) error {
	...
	_, err := s.db.Exec(`
		INSERT INTO sessions (id, project_path, kind, agent_id, claude_session_id, title, status, pinned, scope, archived, deleted_at, notify_on_idle, model, created_at, last_active_at)
		VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
		ON CONFLICT(id) DO UPDATE SET
	...
	`, rec.ID, rec.ProjectPath, rec.Kind, rec.AgentID, rec.ClaudeSessionID,
		rec.Title, rec.Status, pinned, scope, archived, deletedAt, notify, rec.Model, created, active)
	return err
}
...
func (s *Store) ListSessions() ([]SessionRecord, error) {
	rows, err := s.db.Query(`
		SELECT id, project_path, kind, agent_id, claude_session_id, title, status, pinned, scope, archived, deleted_at, notify_on_idle, model, created_at, last_active_at
		FROM sessions
...
func scanSession(r scanRow) (SessionRecord, error) {
	...
	if err := r.Scan(&rec.ID, &rec.ProjectPath, &rec.Kind, &rec.AgentID,
		&rec.ClaudeSessionID, &rec.Title, &rec.Status, &pinned, &rec.Scope, &archived, &deletedAt, &notify, &rec.Model, &createdAt, &activeAt); err != nil {
