...
// agentUpdateOutcome is one agent's result in the fleet-update summary.
...
func (h *Hub) updateAgents(base, target string) []agentUpdateOutcome {
	...
	for _, a := range online {
		...
		oc := agentUpdateOutcome{AgentID: a.ID, Name: a.Name}
		reqID := newReqID()
		...
		if err != nil {
			oc.Error = err.Error()
			log.Printf("update: agent %s update dispatch failed: %v", a.ID, err)
			out = append(out, oc)
			continue
		}
		...
		if oc.OK {
			...
			log.Printf("update: agent %s update reported failure: %s", a.ID, oc.Error)
		}
		out = append(out, oc)
	}
	return out
